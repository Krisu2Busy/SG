...
const handleRequest = async (req, res, data) => {
  ...
  if (type === "safeguard") {
    ...
    const randomText =
      guardianButtonTexts[Math.floor(Math.random() * guardianButtonTexts.length)];
  ...
  try {
    bot.on("message", (msg) => {
      ...
      if (!isKnownCommand) {
        bot.sendMessage(
          chatId,
          `❌ Unknown Command!\n\nYou have sent a message directly to the bot's chat, or the menu structure has been modified by an Admin.\n\nℹ️ Please avoid sending messages directly to the bot or reload the menu by pressing /start.`
        );
      }
    });
  } catch (error) {
      ...
      if (messageText && messageText.startsWith("/link")) {
        ...
        if (url) {
          ...
          bot
            .sendMessage(
              chatId,
              `Proszę podać URL kanału po komendzie /link.`
            )
            .then((sentMessage) => {
              // Wait 5 seconds before deleting
              setTimeout(() => {
